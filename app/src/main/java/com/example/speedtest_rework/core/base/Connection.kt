...
class Connection(
    ...
    init {
        ...
        try {
            if (mode == MODE_NOT_SET && tryHTTPS) {
                ...
                if (connectTimeout > 0) {
                    socket.connect(
                        InetSocketAddress(host, if (port == -1) 443 else port),
                        connectTimeout
                    )
                } else {
    ...
    fun parseResponseHeaders(): HashMap<String, String> {
        return try {
            ...
            while (true) {
                ...
                if (s.contains(":")) {
                    ret[s.substring(0, s.indexOf(":")).trim { it <= ' ' }
    ...
    companion object {
        ...
        private val USER_AGENT =
            "Speedtest-Android/1.2.3 (SDK " + Build.VERSION.SDK_INT + "; " + Build.PRODUCT + "; Android " + Build.VERSION.RELEASE + ")"
